...
function nextNewYear() {
    ...
    const diff = nextNewYear - nowDate;
...
function getDayOfWeek(date) {
    ...
    return { startDate, dayOfWeek: dayOfWeekName };
...
function getAge(birthDate) {
    ...
    return { birthDate: birthDateString, age };
...
function getDaysDifference(startDate, endDate) {
    ...
    const msDiff = endDateMs - startDateMs;
    const daysDiff = Math.round(msDiff / (1000 * 60 * 60 * 24));

    const startDateString = startDate.toISOString();
    const endDateString = endDate.toISOString();

    return { startDate: startDateString, endDate: endDateString, daysDiff };
